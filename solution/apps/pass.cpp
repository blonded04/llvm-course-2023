...
private:
    ...
    using PatternStats = std::map<Pattern, unsigned>;
    ...
    bool runOnFunction(const Function &function) override {
        ...
        for (const auto &basic_block : function) {
            for (const auto &instruction : basic_block) {  
                ...
                for (unsigned window_size = k_analyze_min_window_size; window_size <= k_analyze_max_window_size; window_size++) {
                    analyzeInstruction(instruction, window_size, stats[window_size - k_analyze_min_window_size]);
                }
            }
        }
        ...
        for (unsigned window_size = k_analyze_min_window_size; window_size <= k_analyze_max_window_size; window_size++) {
            std::vector<PatternStat> most_frequent_patterns(stats[window_size].begin(), stats[window_size].end());
            std::sort(most_frequent_patterns.begin(), most_frequent_patterns.end(), [](const PatternStat &lhs, const PatternStat &rhs) {
                return lhs.second > rhs.second;
            });
            
            outs() << "\tFor window of size " << window_size << " most frequent patterns are:\n";
            for (unsigned i = 0u; i < std::max(most_frequent_patterns.size(), k_stats_max_patterns_output_size); i++) {
